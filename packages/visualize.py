...
def xmatch_mode_breakdown(battles: pd.DataFrame, locale: Locale = Locale.JA):
    ...
    mode_count = (
        battles["mode"].value_counts().reset_index().set_axis(["mode", "count"], axis=1)
    )
...
def xmatch_power_distribution(battles: pd.DataFrame, locale: Locale = Locale.JA):
    ...
    i18n.add(
        "title",
        "集計した stat.ink 投稿者のXパワー分布（バトル数ベース）",
        "X Power distribution of stat.ink users (based on number of battles)",
    )
...
def splatfest_challenge_power_distribution(
    battles: pd.DataFrame, locale: Locale = Locale.JA
):
    invalid_battle_num = len(battles[battles["power"].isna()])
    print(f"パワー不明バトル数: {invalid_battle_num}")
    power_agg = battles["power"].describe()
    ...
    i18n.add(
        "title",
        "集計した stat.ink 投稿者のフェスパワー分布（バトル数ベース）",
        "Splatfest power distribution of stat.ink users (based on number of battles)",
    )
...
def event_power_distribution(battles: pd.DataFrame, locale: Locale = Locale.JA):
    invalid_battle_num = len(battles[battles["power"].isna()])
    print(f"パワー不明バトル数: {invalid_battle_num}")
    power_agg = battles["power"].describe()
    ...
    i18n.add(
        "title",
        "集計した stat.ink 投稿者のイベントパワー分布（バトル数ベース）",
        "Event power distribution of stat.ink users (based on number of battles)",
    )
    i18n.add("power", "イベントパワー", "Event power")
    i18n.add("battle_num", "バトル数", "Number of battles")
    i18n.add("mean", "平均値", "Average")
    i18n.add("sd", "標準偏差", "Standard deviation")
